...
/* В файле пароль и токен, поэтому посторонним читать его нельзя.
   Но сайт работает не от root, а от www-data, и без группы веб-сервера
   он получит «Permission denied» прямо при чтении настроек. Группу берём
   у каталога сайта — там она уже правильная, гадать не нужно. */
$group = @filegroup(__DIR__);
if ($group !== false) {
    @chgrp($target, $group);
}
@chmod($target, 0640);

// Проверяем глазами скрипта сайта: сможет ли он открыть файл
if (!is_readable($target)) {
    echo "\nВнимание: файл записан, но прочитать его не удаётся.\n";
    echo "Выполните на сервере:\n";
    echo "    chgrp www-data $target && chmod 640 $target\n";
}
