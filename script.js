...
(function () {
  ...
  function pickSolution(a) {
    ...
    if (has("safety")) {
      hardware = "Противовзломная фурнитура — дополнительная комплектация";
      hardwareNote = "В базе идёт поворотно-откидная фурнитура с микропроветриванием. Противовзломную ставим по запросу: она подбирается индивидуально под размер и вес створки — обсудим на замере.";
      why.push("вы отметили безопасность — противовзломная фурнитура идёт дополнительной комплектацией, замерщик подберёт её под ваши створки");
    } else {
